...
패키지 만들기 - 클래스 파일이 저장되는 위치는 클래스나 인터페이스가 컴파일되면 클래스 파일 생성, 클래스 파일은 패키지로 선언된 디렉토리에 저장
...
서브 클래스/슈퍼 클래스의 생성자 호출과 실행 - 서브 클래스의 객체가 생성될 때(슈퍼 클래스 생성자와 서브 클래스 생성자 모두 실행)
